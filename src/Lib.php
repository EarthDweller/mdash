...
class Lib
{
    ...
    {
    	if ($way)
            $text = preg_replace_callback('/(\<\/?)([^<>]+?)(\>)/s', function($m){return (strlen($m[1])==1 && substr(trim($m[2]), 0, 1) == '-' && substr(trim($m[2]), 1, 1) != '-')? $m[0] : $m[1].( substr(trim($m[2]), 0, 1) === "a" ? "%%___"  : ""  ) . Lib::encrypt_tag(trim($m[2]))  . $m[3];}, $text);
        else
            $text = preg_replace_callback('/(\<\/?)([^<>]+?)(\>)/s', function($m){return (strlen($m[1])==1 && substr(trim($m[2]), 0, 1) == '-' && substr(trim($m[2]), 1, 1) != '-')? $m[0] : $m[1].( substr(trim($m[2]), 0, 3) === "%%___" ? Lib::decrypt_tag(substr(trim($m[2]), 4)) : Lib::decrypt_tag(trim($m[2])) ) . $m[3];}, $text);
        return $text;
    }
    ...
    {
        $text = preg_replace_callback('/'.Lib::INTERNAL_BLOCK_OPEN.'([a-zA-Z0-9\/=]+?)'.Lib::INTERNAL_BLOCK_CLOSE.'/s', function($m){return EMT_Lib::decrypt_tag($m[1]);}, $text);
        return $text;
    }
	...
	public static function convert_html_entities_to_unicode(&$text)
	{
        $text = preg_replace_callback("/\&#([0-9]+)\;/",
            function($m) {return Lib::_getUnicodeChar(intval($m[1]));}
            , $text);
        $text = preg_replace_callback("/\&#x([0-9A-F]+)\;/",
            function($m) {return Lib::_getUnicodeChar(hexdec($m[1]));}
            , $text);
        $text = preg_replace_callback("/\&([a-zA-Z0-9]+)\;/",
            function($m) {$r = Lib::html_char_entity_to_unicode($m[1]); return $r ? $r : $m[0];}
            , $text);
	}
